...
let ProductionSafetyForm = (props, ref) => {
  ...
  const onDownload = (type) => {
    window.open(`${baseURL}/help/enterprise/table/exportWord/enterpriseBaseInfo?entCode=${getCookie('entCode')}&exportType=8&access_token=${getCookie("access_token")}`)
  }
  return (
    <Spin spinning={loading}>
      <Collapse defaultActiveKey={['1', '2']} expandIconPosition='end'
        expandIcon={({ isActive }) => <RightOutlined rotate={isActive ? 270 : 90} />}>
        <Panel header={BaseHeader('消防器材、防护配置情况登记表', '')} key="1" collapsible='disabled' showArrow={false} extra={isEdit ? genEditExtra() : genSaveExtra()}>
