...
import { getInputForm, saveInputForm } from '@/api/info'
import AnswerTable from './inputTable'
...
let AccidenttForm = (props, ref) => {
  ...
  const onFinish = async (values) => {
    try {
      ...
      await saveInputForm(getCookie('entCode'), timeArray).then(res => {
  ...
  const initBaseInfo = async () => {
    let res = await getInputForm(getCookie('entCode'))
    ...
    if (personDistributionSituation.length === 0) {
      ...
      personDistributionSituation.map(item => {
        ...
        if (item.datetime) item.datetime = (moment(item.datetime, 'YYYY-MM-DD'))
        else item.datetime = ''
        return item
      })
    }
  ...
  const onDownload = (type) => {
    window.open(`${baseURL}/help/enterprise/table/exportWord/enterpriseBaseInfo?entCode=${getCookie('entCode')}&exportType=11&access_token=${getCookie("access_token")}`)
  }
  return (
    <Spin spinning={loading}>
      <Collapse defaultActiveKey={['1', '2']} expandIconPosition='end'
        expandIcon={({ isActive }) => <RightOutlined rotate={isActive ? 270 : 90} />}>
        <Panel header={BaseHeader('安全生产投入登记表', '')} key="1" showArrow={false} collapsible='disabled' extra={isEdit ? genEditExtra() : genSaveExtra()}>
